...
1. **Dual `:root` token palettes (globals.css line 1 vs line 3527).** Two sources of truth for brand color. Any future edit to the top palette silently does nothing. **Not auto-fixed** (collapsing them risks a color shift if any variable is defined in only one block). Recommend manually merging into one `:root` after confirming every token's effective value.
2. **Dark mode is defined but neutralized.** `@media (prefers-color-scheme: dark)` (line 106) sets dark values for `--background/--panel/--ink/--muted`, but the later unconditional Gen-2 `:root` (line 3527) resets them to light hex values, so users on a dark OS get the light theme with a broken/partial cascade. Either finish dark mode or delete the dead media block. **Not auto-fixed** (intent unclear).
3. **`!important` arms race.** After dead-code removal there are still **505** `!important` in globals, **142** in practice, **204** in tests (851 total). Most exist to force Gen-2 over Gen-1; with Gen-1 gone, most are now unnecessary but removing them is a per-rule verification job (see Refactoring).
...
- **`!important` sweep:** remove them in batches per component section, rebuild, and diff-screenshot. Expect the large majority to be droppable now.
