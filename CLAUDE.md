...
- `content/nsb/json/questions/**` — the monolith split by `(category, schoolLevel)` into 12 shards (`<subject-slug>__<LEVEL>.json`, ~1–4 MB each) + `manifest.json`, generated from `questions.json` by `scripts/shard-questions.mjs` (`npm run generate:question-shards`). The shards are committed (same "all content is committed files" model) and are what the **runtime** reads; `questions.json` stays the committed source of truth for the content-pipeline scripts and the explanation backfill, and is intentionally **not** imported at runtime. Regenerate the shards whenever `questions.json` changes.

Loaders live in `src/data/nsbQuestions.ts`. They read question **shards** (not the monolith) via `fs.readFile` on `path.join(process.cwd(), "content/nsb/json/questions", …)`: `loadShard()`/`loadRawQuestions()` read only the shard(s) matching a subject+level and memoize per shard in a module-scope `Map`. `getNsbQuestionsFor(subject, level)` is the hot path (loads just the needed shard[s]); `getNsbQuestions()` composes all 12 shards for callers that must scan the whole bank (question-by-id, buzzer pool, yield stats) and memoizes the result. `getNsbLessonContent()` also uses `fs.readFile`. This is why `next.config.ts` must keep **both** `outputFileTracingIncludes` entries — `./content/nsb/lessons/**` **and** `./content/nsb/json/questions/**`. **Never remove either** or the corresponding loader silently returns `[]` on Vercel. `contentPath` in `lessons.json` is repo-root-relative (`content/nsb/lessons/{hs,ms}/...`) and is resolved via `path.join(process.cwd(), contentPath)`.

**All content reads go through `src/lib/data.ts`.** Every reader special-cases `competitionSlug === "science-bowl"` (NSB path) and falls back to local TypeScript arrays in `src/data/` for other competitions. Those arrays are intentionally empty (`practiceQuestions.ts`, `lessons.ts`). There is no database branch. **Caching:** the derived count readers (`getContentCounts*`) and `getQuestionsForLesson` are wrapped in Next `unstable_cache` (tag `nsb-content`) via a local `cached()` helper that falls back to running the fn directly when there's no incremental cache (tests/scripts) — so the suite stays hermetic. Random results are **not** cached with a stable key; randomness lives in `GET /api/practice/random` (`force-dynamic`), which the static practice-page shells fetch client-side. There is no `/api/revalidate` endpoint.
